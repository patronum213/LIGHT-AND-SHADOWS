...
repeat (_move_count)
{
	...
	{
		colliding_object = instance_place(x+_move_once, y, all);
...
repeat (_move_count)
{
...
else if (colliding_object == RGrappleable_object_parent) {
	...
	if (!place_meeting(x, y, player)) {
		if (abs(x-player.x) >= abs(y-player.y)) {
			player.vel_x = pull_force*(x-player.x)/abs(x-player.x);
			player.vel_y = pull_force*(y-player.y)/abs(x-player.x);
		}
		else if (abs(x-player.x) <= abs(y-player.y)) {
			player.vel_x = pull_force*(x-player.x)/abs(y-player.y);
			player.vel_y = pull_force*(y-player.y)/abs(y-player.y);
		}
	}
	else {
		player.vel_x = 0;
...
if (keyboard_check_pressed(vk_space)) {player.grappling = false;instance_destroy(id);}