...
vel_y += grav;
if (collided) {
	if (delay < 5) {image_index = 1;}
	delay = delay > 0 ? delay - 1 : 0;
	}
if (!player.grounded and collided and delay == 0) {
	distance_raw = sqrt(sqr(x-player.x)+sqr(y-player.y))
	multiplier = 1/(distance_raw/(distance_normalising/curving_value))*curving_value > 1 ? 1 : 1/(distance_raw/(distance_normalising/curving_value))*curving_value;
	if (abs(x-player.x) >= abs(y-player.y)) {
		player.vel_x += (force*(x-player.x)/abs(x-player.x))*multiplier;
		player.vel_y += (force*(y-player.y)/abs(x-player.x))*multiplier;
	}
	else if (abs(x-player.x) <= abs(y-player.y)) {
		player.vel_x += (force*(x-player.x)/abs(y-player.y))*multiplier;
		player.vel_y += (force*(y-player.y)/abs(y-player.y))*multiplier;
		
	}
	instance_create_layer(x, y, layer, explosion);
	instance_destroy(id);
}
else if (player.grounded and collided and delay == 0) {instance_create_layer(x, y, layer, explosion); instance_destroy(id);}
